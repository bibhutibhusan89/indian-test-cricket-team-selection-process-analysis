...
getCricketData <- function (inputID){
  ...
  # replacing comma with pipe character in the state name
  player_dob_state=gsub(",", "|", player_dob_state)
  ...
  if(mortal_status=="Died"){
    ...
    player_age=as.numeric(player_dod_year) - as.numeric(player_dob_year)
    player_age=paste(player_age," years 0 days")
  }else{
  ...
  p_info=cbind(player_name,player_dob_mm,player_dob_dd,player_dob_year,
               player_dob_city,player_dob_state,died_flag,player_dod,player_age)
  # returning the constructed player information data frame
  return(p_info)
}
...
# writing the column names to the output CSV file
